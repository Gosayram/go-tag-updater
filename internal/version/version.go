...
func GetFullVersionInfo() string {
	...
	// Main version line
	versionLine := fmt.Sprintf("go-tag-updater %s", GetVersion())
...
func (bi *BuildInfo) String() string {
	return fmt.Sprintf("go-tag-updater version %s (%s) built on %s by %s using %s for %s",
		bi.Version, bi.Commit, bi.Date, bi.BuiltBy, bi.GoVersion, bi.Platform)
}

// Short returns a short version string
func (bi *BuildInfo) Short() string {
	return fmt.Sprintf("go-tag-updater %s", bi.Version)
}
